...
/**
 * Admin receipt detail — preview and print.
 */
export function AdminReceiptDetailPage() {
  ...
  const detailQuery = useQuery({
    ...
    queryFn: async () => {
      const payload = await apiClient.get(`/admin/receipts/${receiptId}`);
      return payload.data;
    },
  });
    ...
    <div>
      <div className="print:hidden">
        <p className="text-sm text-muted">
          <Link to="/admin/receipts" className="hover:underline">
            Receipts
          </Link>
        ...
        {receipt.invoiceId ? (
          <p className="mt-2 text-sm text-muted">
            Invoice:{' '}
            <Link to={`/admin/invoices/${receipt.invoiceId}`} className="hover:underline">
