...
import { softDeleteVehicle, updateVehicle } from '../../../server/services/adminDeliveryService.js';

async function handler(req, res) {
  if (applyCors(req, res)) return;
  const id = req.params?.id || req.query?.id;
  if (!id) {
    const result = failure('Vehicle id is required', { code: 'VALIDATION_ERROR', status: 400 });
  ...
  if (req.method === 'PATCH' || req.method === 'PUT') {
    ...
    json(res, 200, success({ vehicle }));
    return;
  }

  if (req.method === 'DELETE') {
    await requireAnyPermission('delivery.assign', 'delivery.update')(req);
    json(res, 200, success(await softDeleteVehicle(id)));
    return;
  }
